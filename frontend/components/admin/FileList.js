    ...
    const FilterButton = ({ type, label }) => (
        <button
            ...
            className={`px-3 py-1 text-sm font-medium rounded-full ${
                filterType === type ? 'bg-iit-blue text-white' : 'bg-gray-200 hover:bg-gray-300'
            }`}
        >
            {label}
        </button>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {filteredAndSortedFiles.map(file => (
                            <tr key={file.id}>
                                <td className="px-4 py-3 text-sm font-medium text-gray-800 break-all">{file.originalName}</td>
                                <td className="px-4 py-3 text-sm text-gray-600">{file.mimeType}</td>
                                <td className="px-4 py-3 text-sm text-gray-600">{(file.size / 1024 / 1024).toFixed(2)} MB</td>
                                {/* --- ADDED NEW COLUMN DATA --- */}
                                <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{formatDate(file.createdAt)}</td>
                                <td className="px-4 py-3 text-sm">
                                    <button onClick={() => handleCopy(file.url)} className="flex items-center text-iit-blue hover:underline">
                                        {copiedUrl === file.url ? 'Copied!' : 'Copy URL'} <Copy size={14} className="ml-2"/>
                                    </button>
                                </td>
                                <td className="px-4 py-3 text-sm">
                                    <button onClick={() => onDelete(file.id, file.originalName)} className="text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-100">
                                        <Trash2 size={18} />
                                    </button>
                                </td>
                            </tr>
